...
export const LiveScreens: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="bg-surface p-4 rounded-2xl border border-line-soft flex items-center justify-between">
        <h3 className="font-bold text-sm text-fg flex items-center gap-2">
          <MonitorPlay className="w-4 h-4 text-brand" /> Màn Hình Trực Tiếp Đa Luồng
        </h3>
        <span className="text-[11px] text-fg-muted font-semibold">
          Đang chạy: <span className="text-brand font-bold">{ids.length}</span> trình duyệt
        </span>
      </div>

      {ids.length === 0 ? (
        <div className="bg-surface border border-line-soft rounded-2xl py-16 text-center">
          <MonitorPlay className="w-10 h-10 text-slate-700 mx-auto mb-3" />
          <p className="text-sm text-fg-subtle font-semibold">Chưa có trình duyệt nào đang chạy.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
          {ids.map((id) => {
            ...
            return (
              <div
                key={id}
                className="bg-surface border border-line-soft rounded-xl overflow-hidden group relative cursor-zoom-in hover:border-teal-500/50 transition-colors"
                onClick={() => setZoomId(id)}
              >
                <div className="flex items-center justify-between px-3 py-2 border-b border-line-soft bg-surface-2">
                  <span className="text-[11px] font-bold text-fg truncate">{f.username}</span>
                  <span className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-teal-400 animate-pulse" />
                    <Maximize2 className="w-3 h-3 text-fg-subtle group-hover:text-brand" />
                  </span>
                </div>
                <img
        ...
        >
          <div className="relative max-w-5xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold text-fg">{zoomed.username}</span>
              <button
                onClick={() => setZoomId(null)}
                className="p-1.5 rounded-md bg-slate-800 hover:bg-slate-700 text-fg"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
