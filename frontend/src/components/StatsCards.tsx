  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {items.map(({ label, value, Icon, tint, bg }) => (
        <div key={label} className="card px-4 py-3.5 flex items-center justify-between">
          <div className="min-w-0">
            <p className="text-[11px] text-fg-muted font-semibold uppercase tracking-wide truncate">{label}</p>
            <p className="text-2xl font-bold mt-0.5 text-fg tabular-nums">{value}</p>
          </div>
          <div className={`grid place-items-center w-10 h-10 rounded-xl border ${bg} shrink-0`}>
