...
export const ExportModal: React.FC<ExportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-[#0e1424] border border-slate-700 rounded-2xl w-full max-w-lg p-5 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
          <h3 className="font-bold text-slate-100 flex items-center gap-2">
            <Package className="w-5 h-5 text-teal-400" /> Xuất tài khoản ra file .txt
          </h3>
        ...
        <div className="bg-[#141b2e] rounded-xl p-4 mb-3 border border-slate-800">
          <p className="text-sm font-bold text-slate-200 mb-1">1. Xuất tài khoản đã chọn</p>
          <p className="text-xs text-slate-400 mb-3">
            Đang chọn: <span className="text-teal-400 font-bold">{selectedAccountIds.length}</span> tài khoản
          </p>
          <button
            disabled={busy || selectedAccountIds.length === 0}
            onClick={() => doExport(selectedAccountIds, null)}
            className="w-full bg-teal-500 hover:bg-teal-600 disabled:bg-slate-700 disabled:text-slate-500 text-slate-950 font-bold text-sm py-2 rounded-lg flex items-center justify-center gap-2 transition-all"
          >
        ...
        <div className="bg-[#141b2e] rounded-xl p-4 border border-slate-800">
          <p className="text-sm font-bold text-slate-200 mb-1">2. Lấy số lượng từ lô đang hiển thị</p>
          <p className="text-xs text-slate-400 mb-3">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="w-24 bg-[#0e1424] border border-slate-700 rounded-lg p-2 text-center font-bold text-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400"
            />
            <button
              disabled={busy || displayedAccountIds.length === 0}
              onClick={() => doExport(displayedAccountIds, quantity)}
              className="flex-1 bg-indigo-500 hover:bg-indigo-600 disabled:bg-slate-700 disabled:text-slate-500 text-white font-bold text-sm py-2 rounded-lg flex items-center justify-center gap-2 transition-all"
            >
        ...
        {result && (
          ...
          >
            ...
            <p className="text-sm text-slate-100 flex items-center gap-1.5">
              ...
              Đã xuất & tải về: <span className="font-bold text-teal-400 text-lg">{result.exported_count}</span> tài khoản.
            </p>
            <p className="text-sm text-slate-300 mt-1">
              Lô còn lại: <span className="font-bold text-slate-100">{result.remaining_count}</span> tài khoản.
            </p>
        ...
        {busy && <p className="text-xs text-slate-400 mt-3 text-center animate-pulse">Đang xuất...</p>}
      </div>
    </div>
  );
};
